...
- `Serialize` either omits the field (if the containing struct's serialization is only ever used for non-credential purposes) or emits the same fixed redaction — the implementation plan should confirm which existing call sites serialize `EmailConfig` and whether any of them legitimately need the real password to round-trip (e.g., a config file load/save path), in which case the wrapper needs an explicit, separately-named accessor for that one path rather than a blanket `Serialize` impl.
- The wrapped value is still a plain `String` in memory — this slice does not add memory-zeroing-on-drop or any OS-level secret storage. That is a larger, portfolio-wide credential-handling question (every project with a password field has the same gap), not something to solve inside one transport's slice.
...
- Decide, per §5, whether any existing call site needs `EmailConfig`'s real password to round-trip through serialization, and if so design that one accessor explicitly rather than leaving a gap in the wrapper.
