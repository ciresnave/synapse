...
- [ ] **Step 1: Resolve the identity-model question, and record a finding about the split itself**

Read `src/identity.rs`'s `IdentityRegistry` in full (not just `get_public_key`) and `src/sender_auth.rs`'s `TrustStore`/`verify_at` in full. **These are two completely disjoint identity mechanisms in this crate, confirmed by file-level usage (`TrustStore`: `delivery_ack.rs`, `lib.rs`, `mcp_server.rs`, `sender_auth.rs`, `transport/manager.rs`; `IdentityRegistry`: `bin/client.rs`, `identity.rs`, `router.rs`; zero files use both) — not two overlapping models that drifted apart, two systems that have never met.** Note also: `SenderProof` (the type `sign_new_message`, Task 2, actually sets) is itself part of the `sender_auth.rs`/`TrustStore` vocabulary — so the signing half of this router already speaks `TrustStore`'s language while `IdentityRegistry` only ever did key lookup. A bridge between the two is unavoidable either way; the question is where it goes and how strong it is.

Decide, and record your reasoning in this task's report: does the merged router's receive-side verification use `IdentityRegistry::get_public_key` directly with a raw signature check against that key, or adopt a `TrustStore`? **Recommendation, not a mandate:** prefer `IdentityRegistry`, since introducing `TrustStore` as new router state is a larger scope addition this plan did not budget for.

**Required, regardless of which you choose — a recorded finding, not just a local decision:** state explicitly whether verification via your chosen mechanism gives **the same guarantee** `TransportManager`'s `TrustStore`-based verification gives every other transport in this crate, or a **weaker** one. If weaker, say so in those exact terms in your report — a weaker receive-side guarantee on this router than the rest of the crate enforces is itself the two-paths-different-behavior defect this whole plan exists to close, one layer up, and must be reported, not absorbed silently into "the router now checks something."

**Do not converge the two identity models in this plan, even if your research suggests they should be.** If Step 1's research leads you to conclude `IdentityRegistry` and `TrustStore` ought to merge or that the router should adopt `TrustStore` outright, stop and report that as its own finding for the PM to escalate — it is an architectural decision belonging to the project owner, not a step to take inside a router-merge task.
