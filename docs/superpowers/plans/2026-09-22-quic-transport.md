...
- **Condition 3 is NOT met by this plan as executed.** `QuicTransportImpl::evict` exists and is directly tested, but nothing calls it: `TransportManager` has no way to reach a concrete `QuicTransportImpl` from its `Box<dyn Transport>` storage, so no application-layer verification failure ever triggers an eviction. This was a plan-writing error (see the corrected Task 2 Interfaces line and the Self-Review Notes below) caught only at the final whole-branch review, after every task-scoped review had already passed by correctly checking its task against this plan rather than the spec directly. Deferred, by the PM's ruling, to a separate cross-transport slice that changes the `Transport`/`TransportReceive` contract deliberately (touching TCP/WS/HTTP/NAT too) rather than as a side effect of shipping QUIC — tracked as https://github.com/ciresnave/synapse/issues/49, referenced in the PR.
...
- Produces: `QuicTransportImpl::pooled_connection(&self, addr: SocketAddr) -> Result<Arc<quinn::Connection>>` (new private method `send_message` calls instead of connecting fresh every time); `QuicTransportImpl::evict(&self, addr: SocketAddr)`, a mechanism for the eviction spec §3 condition 3 describes — **Task 2 provides and directly tests this method only; nothing calls it, so condition 3 itself is NOT met by this plan.** Wiring it to the manager's verification path needs a `Transport`/`TransportReceive` contract change touching every transport, which is deliberately out of scope for this plan (see the corrected Global Constraints entry and Self-Review Notes below).
...
- **Spec coverage:** §2 (task shape) → Tasks 1-5 above. §3, condition 1 (verifier confinement) → Task 1 Steps 4/9, independently re-verified against the spec text at the final whole-branch review: **holds**. §3, condition 2 (lock-release pattern) → Task 2 Step 5, independently re-verified against the spec text at the final whole-branch review: **holds**. §3, condition 3 (eviction on verification failure) → Task 2 Step 5 provides and tests `evict()` as a mechanism, but **nothing wires it to a real verification failure, so the condition itself is NOT met** — caught only at the final whole-branch review, deferred to a separate cross-transport slice by the PM's ruling. This entry was wrong in every earlier revision of this plan (it read "eviction method, condition 3", conflating "the method exists" with "the condition is satisfied") — flagging the correction here rather than silently fixing it, per this project's own retraction discipline. §4 (delivery claim/ack composition) → Task 1's module doc and `send_message`'s `Sent`-only return; no ack code added anywhere, matching "QUIC needs zero new ack code." §5 (0-RTT) → Task 1's `quic_tls.rs` never enables it; no step in this plan turns it on; independently verified against the installed `quinn-proto` source at the final review (the `TryFrom<Arc<rustls::*Config>>` path this code uses leaves `enable_early_data`/`max_early_data_size` at their disabled defaults). §6 (framing/size limits) → Task 3. §7 (capabilities/metrics) → Task 4. §8 (testing/verification) → Task 5.
- **A single wrong entry in this coverage table (condition 3, above) meant the table was not trustworthy evidence for any of its other entries either** — five task-scoped reviews each correctly checked their task against what this plan said, and the plan was wrong about one thing, so the checklist itself could not be trusted without re-deriving it. The final whole-branch review re-verified conditions 1 and 2 directly against the spec's text rather than against this plan, and both hold, stated as a checked result rather than assumed.
