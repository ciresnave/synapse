...
impl CircuitBreaker {
    ...
    pub async fn record_outcome(&self, outcome: RequestOutcome) {
        ...
        match current_state {
            ...
            CircuitState::HalfOpen => match outcome {
                ...
                RequestOutcome::Failure(_) | RequestOutcome::Timeout => {
                    self.transition_to_open("Failure during half-open test")
                        .await;
                }
            },
            CircuitState::Open => {
    ...
    pub async fn check_external_triggers(&self, metrics: &TransportMetrics) {
        ...
        for trigger in &self.external_triggers {
            ...
            if current_state == CircuitState::Open && trigger.should_recover(&current_state) {
                info!(
                    "External trigger suggests recovery: {}",
                    trigger.description()
                );
                self.transition_to_half_open().await;
                return;
    ...
    async fn should_close_from_half_open(&self) -> bool {
        let history = self.request_history.read().unwrap();
        let half_open_requests: Vec<_> = history
            .iter()
            .rev()
        ...
        let success_count = half_open_requests
            .iter()
...
pub struct LatencyTrigger {
    ...
    recent_samples: Arc<RwLock<VecDeque<Duration>>>,
}
...
impl LatencyTrigger {
    pub fn new(max_latency: Duration, samples_required: usize) -> Self {
        Self {
            max_latency,
            samples_required,
            recent_samples: Arc::new(RwLock::new(VecDeque::new())),
        }
    }

    /// Add a latency sample for analysis
    pub fn add_sample(&self, latency: Duration) {
        let mut samples = self.recent_samples.write().unwrap();
        samples.push_back(latency);

        // Keep only the required number of samples
        while samples.len() > self.samples_required {
            samples.pop_front();
        }
    }
    ...
    pub fn average_latency(&self) -> Option<Duration> {
        let samples = self.recent_samples.read().unwrap();
        if samples.is_empty() {
            return None;
        }

        let total_nanos: u64 = samples.iter().map(|d| d.as_nanos() as u64).sum();
        let avg_nanos = total_nanos / samples.len() as u64;
...
impl CircuitTrigger for LatencyTrigger {
    fn should_trip(&self, metrics: &TransportMetrics, state: &CircuitState) -> bool {
        ...
        // Add current latency sample
        self.add_sample(metrics.latency);

        // Check if we have enough samples
        let samples = self.recent_samples.read().unwrap();
        if samples.len() < self.samples_required {
    ...
    fn should_recover(&self, _state: &CircuitState) -> bool {
        // Check if recent samples show improved latency
        if let Some(avg_latency) = self.average_latency() {
            avg_latency <= self.max_latency
        } else {
            false
    ...
    fn description(&self) -> String {
        format!(
            "Latency trigger (max: {:?}, samples: {})",
            self.max_latency, self.samples_required
        )
    }
}
...
impl CircuitBreakerManager {
    ...
    pub async fn check_all_breakers(
        &self,
        metrics: &std::collections::HashMap<String, TransportMetrics>,
    ) {
...
mod tests {
    ...
    async fn test_circuit_breaker_basic_flow() {
        ...
        for _ in 0..3 {
            assert!(breaker.can_proceed().await);
            breaker
                .record_outcome(RequestOutcome::Failure("test error".to_string()))
                .await;
        }
