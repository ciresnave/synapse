...
impl UdpTransport {
    ...
    pub async fn start_server(
        &self,
        mut message_handler: mpsc::UnboundedReceiver<(String, SecureMessage)>,
    ) -> Result<()> {
            ...
            loop {
                match socket_clone.recv_from(&mut buffer).await {
                    Ok((size, peer_addr)) => {
                        ...
                        match bincode::decode_from_slice::<SecureMessage, _>(
                            &buffer[..size],
                            ...
                            Err(e) => {
                                warn!(
                                    "Failed to deserialize UDP message from {}: {}",
                                    peer_addr, e
                                );
                            }
                        }
    ...
    pub async fn discover_local_peers(&self) -> Result<Vec<SocketAddr>> {
        ...
        self.socket
            .send_to(discovery_message, broadcast_addr)
            .await?;
    ...
    async fn send_message(&self, target: &str, message: &SecureMessage) -> Result<String> {
        ...
        match self.socket.send_to(&serialized, addr).await {
            Ok(bytes_sent) => {
                ...
                debug!(
                    "Sent UDP message to {} ({} bytes, {:?})",
                    addr, bytes_sent, duration
                );
