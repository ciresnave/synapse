...
mod http_impl {
    ...
    impl HttpTransportImpl {
        ...
        pub async fn new(config: &HashMap<String, String>) -> Result<Self> {
            ...
            let client = ClientBuilder::new()
                ...
                .build()
                .map_err(|e| {
                    SynapseError::TransportError(format!("Failed to create HTTP client: {}", e))
                })?;
        ...
        async fn start_server(&self) -> Result<()> {
            ...
            let server_addr = format!("{}:{}", self.config.server_address, self.config.server_port)
                .parse::<SocketAddr>()
                .map_err(|e| {
                    SynapseError::TransportError(format!("Invalid server address: {}", e))
                })?;
        ...
        async fn send_http_request(
            ...
            if status_code.is_success() {
                info!(
                    "HTTP message sent successfully to {} ({}ms)",
                    url,
                    latency.as_millis()
                );
                Ok(DeliveryReceipt {
                    ...
                    metadata: {
                        ...
                        metadata
                            .insert("status_code".to_string(), status_code.as_u16().to_string());
        ...
        fn parse_target_url(&self, target: &TransportTarget) -> Result<String> {
            ...
            let host = target.address.as_ref().unwrap_or(&target.identifier);
    ...
    impl Transport for HttpTransportImpl {
        ...
        async fn test_connectivity(&self, target: &TransportTarget) -> Result<ConnectivityResult> {
            ...
            match response {
                Ok(resp) if resp.status().is_success() => Ok(ConnectivityResult {
                    connected: true,
                    rtt: Some(latency),
                    error: None,
                    quality: 1.0,
    ...
    impl TransportFactory for HttpTransportFactory {
        ...
        fn validate_config(&self, config: &HashMap<String, String>) -> Result<()> {
            ...
            {
                return Err(SynapseError::Config("Invalid timeout value".to_string()));
            }
            ...
            {
                return Err(SynapseError::Config("Invalid max message size".to_string()));
            }
...
// Export the HTTP transport implementation
pub use http_impl::*;
