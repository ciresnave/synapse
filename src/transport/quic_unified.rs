...
pub struct QuicTransportImpl {
    ...
    #[allow(dead_code)] // Reserved for future circuit breaker integration
...
impl QuicTransportImpl {
    ...
    async fn connect_to_server(&self, target_addr: SocketAddr) -> Result<QuicConnection> {
        ...
        debug!(
            "QUIC connection established to {} in {:?}",
            target_addr,
            start_time.elapsed()
        );
        Ok(connection)
    ...
    async fn send_quic_message(
        ...
        let mut connection = connection
            .ok_or_else(|| crate::error::SynapseError::NetworkError("Network error".to_string()))?;
        ...
        if message_json.len() > self.max_message_size {
            return Err(crate::error::SynapseError::InvalidMessageFormat(
        ...
        debug!(
            "Sending QUIC message to connection {}: {} bytes",
            connection_id,
            message_json.len()
        );
    ...
    async fn update_metrics(&self, operation: &str, duration: Duration, success: bool) {
        ...
        metrics
            .custom_metrics
            .insert("total_streams".to_string(), total_streams as f64);
        metrics.custom_metrics.insert(
            "max_concurrent_streams".to_string(),
            self.max_concurrent_streams as f64,
        );
...
impl Transport for QuicTransportImpl {
    ...
    async fn send_message(
            ...
            if connections.contains_key(&target.identifier) {
                ...
                if let Some(address) = &target.address {
                    let target_addr = address
                        .parse::<SocketAddr>()
                        .map_err(crate::error::SynapseError::AddressParse)?;

                    let connection = self.connect_to_server(target_addr).await?;
                    connection.id
                } else {
                    return Err(crate::error::SynapseError::NotFound(
                        "Target address not found".to_string(),
                    ));
                }
            }
        };
        ...
        let send_result = timeout(
            ...
            self.send_quic_message(&connection_id, message),
        )
        .await
        ...
        match send_result {
            Ok(duration) => {
                ...
                Ok(DeliveryReceipt {
                    message_id: message.message_id.clone().to_string(),
    ...
    async fn receive_messages(&self) -> Result<Vec<IncomingMessage>> {
        let receive_result = self.receive_quic_messages().await;
                        ...
                        Ok(connection) => {
                            ...
                            Ok(ConnectivityResult {
                                ...
                                details: {
                                    ...
                                    map.insert(
                                        "quic_rtt_ms".to_string(),
                                        quic_rtt.as_millis().to_string(),
                                    );
                                    map.insert("protocol".to_string(), "QUIC".to_string());
                                    map
                                },
