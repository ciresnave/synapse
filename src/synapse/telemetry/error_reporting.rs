...
impl ErrorTelemetry {
    ...
    pub async fn send_batch_to_remote(&self, _batch: &[ErrorReport]) -> Result<(), anyhow::Error> {
        if let Some(endpoint) = &self.config.remote_endpoint {
            ...
            // Removed feature gating
            {
                use tracing::warn;
                warn!(
                    "HTTP feature not enabled, cannot upload events to {}",
                    endpoint
                );
            }
        }
...
mod tests {
    ...
    use super::{ErrorSeverity, ErrorSource, ErrorTelemetry, ErrorTelemetryConfig};
