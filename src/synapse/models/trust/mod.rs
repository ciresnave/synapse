...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
// Manual bincode serialization required for HashMap fields
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
// Manual bincode serialization required for VerificationLevel, Option<VerificationMethod>, Vec<VerifiedClaim>
...
/// Aggregated trust ratings for a participant
...
impl TrustBalance {
    pub fn new(participant_id: String) -> Self {
        Self {
            ...
            staked_points: 0,
            earned_lifetime: 0,
            last_activity: DateTimeWrapper::new(chrono::Utc::now()),
    ...
    pub fn award_points(&mut self, points: u32, reason: &str) {
        ...
        self.last_activity = DateTimeWrapper::new(chrono::Utc::now());
    ...
    pub fn apply_decay(&mut self) -> u32 {
        let _now = DateTimeWrapper::new(chrono::Utc::now());
        let days_since_activity =
            { (_now.into_inner() - self.last_activity.clone().into_inner()).num_days() };
    ...
    pub fn reputation_weight(&self) -> f64 {
        ...
        (self.total_points as f64 / 100.0).sqrt().clamp(0.1, 2.0)
    }
}
...
impl NetworkTrustRating {
    ...
    pub fn calculate_network_score(&mut self) -> f64 {
        ...
        score += f64::min(days_old / 365.0 * 2.0, 5.0); // Up to 5 points for being around
        ...
        self.network_score = score.clamp(0.0, 100.0);
        self.last_calculated = DateTimeWrapper::new(chrono::Utc::now());
...
impl Default for NetworkTrustRating {
    fn default() -> Self {
        Self {
            ...
            last_calculated: DateTimeWrapper::new(chrono::Utc::now()),
        }
    }
}
...
impl TrustCalculator {
    ...
    pub fn calculate_composite_trust(
        ...
        let network_proximity = target_trust
            .network_proximity
...
impl Serialize for EntityTrustRatings {
        ...
        S: serde::Serializer,
    {
        ...
        let received_ratings: HashMap<String, DirectTrustScore> = self
            .received_ratings
            .iter()
            .map(|entry| (entry.key().clone(), entry.value().clone()))
            .collect();
        let given_ratings: HashMap<String, DirectTrustScore> = self
            .given_ratings
            .iter()
            .map(|entry| (entry.key().clone(), entry.value().clone()))
            .collect();
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for EntityTrustRatingsVisitor {
                ...
                V: MapAccess<'de>,
            {
                ...
                let received_ratings =
                    received_ratings.ok_or_else(|| de::Error::missing_field("received_ratings"))?;
                let given_ratings =
                    given_ratings.ok_or_else(|| de::Error::missing_field("given_ratings"))?;
                let average_received =
                    average_received.ok_or_else(|| de::Error::missing_field("average_received"))?;
                let total_ratings_received = total_ratings_received
                    .ok_or_else(|| de::Error::missing_field("total_ratings_received"))?;
                let last_updated =
