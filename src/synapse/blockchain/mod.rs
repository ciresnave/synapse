...
pub use block::{Block, StakePurpose, Transaction, TrustReport, TrustReportType};
...
impl SynapseBlockchain {
    ...
    pub async fn new(config: BlockchainConfig) -> Result<Self> {
        ...
        let consensus_engine = Arc::new(ConsensusEngine::new(config.clone()));
    ...
    pub async fn start_consensus(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            let mut interval =
                tokio::time::interval(tokio::time::Duration::from_secs(config.block_time_seconds));
            loop {
                interval.tick().await;
        ...
        info!(
            "Consensus engine started with {} second block time",
    ...
    pub async fn submit_trust_report(
        ...
        if !self
            ...
            .has_sufficient_stake(
                reporter_id,
                self.config.staking_requirements.min_stake_for_report,
            )
            .await?
        {
        ...
        self.participant_nonces
            .insert(reporter_id.to_string(), nonce);
    ...
    pub async fn get_trust_score(&self, participant_id: &str) -> Result<f64> {
        ...
        // Only declare and use these variables if chrono is enabled
        ...
        let _processed: Vec<String> = Vec::new();
                ...
                if balance.last_activity.clone().into_inner() < cutoff_time {
                    ...
                    if decay_amount > 0 {
                        ...
                        self.staking_manager
                            .update_balance(&participant_id, &balance)
                            .await?;

                        info!(
                            "Applied decay to {}: -{} points ({} days inactive)",
                            participant_id, decay_amount, days_inactive
                        );
                    }
                }
            }
        ...
        // Return average trust score if any reports, else 0.0
        if report_count > 0 {
    ...
    pub async fn get_stats(&self) -> Result<BlockchainStats> {
        ...
        Ok(BlockchainStats {
            ...
            total_stakes,
            last_block_time: chain.last().map(|b| DateTimeWrapper::new(b.timestamp.0)),
        })
    }
}
...
pub struct BlockchainStats {
    ...
    pub last_block_time: Option<DateTimeWrapper>,
}
