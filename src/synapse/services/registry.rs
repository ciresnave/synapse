...
impl ParticipantRegistry {
    /// Search for participants by email address
    pub async fn search_participants_by_email(
        &self,
        email: &str,
    ) -> Result<Vec<ParticipantProfile>> {
        let all_participants = self.database.as_ref().get_all_participants().await?;
        let mut results = Vec::new();
        for profile in all_participants {
            if profile
                .identities
                .iter()
                .any(|id| id.email_address.as_deref() == Some(email))
            {
    ...
    pub async fn get_participant(&self, global_id: &str) -> Result<Option<ParticipantProfile>> {
        ...
        if let Ok(Some(cached)) = self
            .cache
            .get_cached::<ParticipantProfile>(&cache_key)
            .await
        {
    ...
    pub async fn search_participants(
        ...
        // Try cache first
        if let Ok(Some(cached_ids)) = self.cache.get_cached_search_results(&cache_key).await {
            let mut results = Vec::new();
        ...
        let mut results = self
            .database
            .search_participants(&query.query, &query.requester_id, query.max_results)
            .await
        ...
        let _ = self
            .cache
            .cache_search_results(&cache_key, &result_ids, 600)
            .await; // 10 min TTL
    ...
    pub async fn get_participants_by_organization(
        &self,
        organization: &str,
    ) -> Result<Vec<ParticipantProfile>> {
        // Generate cache key
        ...
        let results = self
            .database
            .query_participants(sql_query, &[organization])
            .await
        ...
        let _ = self
            .cache
            .cache_participant(&cache_key, &result_ids, 600)
            .await; // 10 min TTL
    ...
    pub async fn get_participants_by_topic(&self, topic: &str) -> Result<Vec<ParticipantProfile>> {
        ...
        // Try cache first
        if let Ok(Some(cached_ids)) = self.cache.get_cached::<Vec<String>>(&cache_key).await
            && let Ok(Some(result)) = self.get_participant(&cached_ids[0]).await
        {
    ...
    pub async fn get_participant_by_alias(
        ...
        let mut results = self
            .database
            .query_participants(sql_query, &[alias])
            .await
    ...
    /// Get reference to the database for statistics queries
    pub fn get_database(&self) -> &Database {
        &self.database
    }

    /// Delete a participant from the registry
    pub async fn delete_participant(&self, participant_id: &str) -> Result<bool> {
        // Remove from database
        match self.database.delete_participant(participant_id).await {
            Ok(()) => {
                info!("Deleted participant: {}", participant_id);
                Ok(true)
            }
            Err(e) => {
                warn!("Failed to delete participant {}: {}", participant_id, e);
                Ok(false)
            }
        }
    }
