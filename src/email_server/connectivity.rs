...
pub enum ServerRecommendation {
    ...
    RelayOnly { reason: String },
    /// Use external email provider
    ExternalProvider { reason: String },
}
...
impl ConnectivityDetector {
    ...
    async fn test_external_accessibility(&self) -> FirewallStatus {
        ...
        if timeout(Duration::from_secs(1), TcpListener::bind("127.0.0.1:25"))
            .await
            .is_ok()
        {
    ...
    fn determine_recommendation(
        ...
        match (can_bind_smtp, can_bind_imap, external_ip, firewall_status) {
            ...
            (true, _, _, FirewallStatus::Blocked) => ServerRecommendation::RelayOnly {
                reason: "Ports available locally but blocked by firewall/NAT".to_string(),
            },

            // Can't bind to ports (likely running as non-root)
            (false, false, _, _) => ServerRecommendation::ExternalProvider {
                reason: "Unable to bind to email server ports (try running as administrator)"
                    .to_string(),
            },

            // No external IP
            (_, _, None, _) => ServerRecommendation::RelayOnly {
                reason: "No external IP address detected".to_string(),
            },

            // Unknown firewall status - be conservative but allow local use
            (true, _, _, FirewallStatus::Unknown) => ServerRecommendation::RelayOnly {
                reason: "Unable to determine external accessibility - running in local mode"
                    .to_string(),
            },

            // Can bind to IMAP only but not SMTP
            (false, true, Some(_), _) => ServerRecommendation::ExternalProvider {
                reason: "Can bind to IMAP but not SMTP - consider using external provider"
                    .to_string(),
            },
        }
    }
    ...
    pub async fn test_external_port(&self, port: u16) -> Result<bool> {
        ...
        let _listener = timeout(self.test_timeout, TcpListener::bind(addr))
            ...
            .map_err(|e| {
                SynapseError::NetworkError(format!("Failed to bind to port {port}: {e}"))
            })?;
