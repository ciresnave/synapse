...
use crate::{error::Result, transport::abstraction::TransportMetrics};
...
impl MetricsCollector {
    ...
    pub async fn generate_health_report(&self) -> HealthReport {
        ...
        let overall_health =
            Self::calculate_health_score(&system_metrics, &transport_metrics, &active_alerts);
    ...
    fn generate_recommendations(
        ...
        if system_metrics.cpu_usage_percent > 80.0 {
            recommendations.push(
                "Consider scaling up CPU resources or optimizing message processing".to_string(),
            );
        }

        if system_metrics.error_rate_percent > 5.0 {
            recommendations.push(
                "High error rate detected - check transport reliability and network conditions"
                    .to_string(),
            );
        }
        ...
        if system_metrics.average_response_time > Duration::from_millis(500) {
            recommendations
                .push("Consider enabling UDP transport for low-latency scenarios".to_string());
        }

        if active_alerts
            .iter()
            .any(|a| matches!(a.alert_type, AlertType::CircuitBreakerOpen))
        {
            recommendations.push(
                "Circuit breakers are open - check network connectivity and target availability"
                    .to_string(),
            );
        }

        let total_failed = transport_metrics
            .values()
            .map(|m| m.send_failures)
            .sum::<u64>();
        let total_sent = transport_metrics
            .values()
            .map(|m| m.messages_sent)
            .sum::<u64>();
        if total_sent > 0 && (total_failed as f64 / total_sent as f64) > 0.1 {
            recommendations.push(
                "High transport failure rate - consider enabling additional transport redundancy"
                    .to_string(),
            );
        }
...
impl PerformanceHistory {
    ...
    fn add_sample(&mut self, metrics: SystemMetrics) {
        ...
        self.message_rates
            .push_back((timestamp, metrics.message_throughput_per_second));
        if self.message_rates.len() > self.max_samples {
            self.message_rates.pop_front();
        }

        self.latency_samples
            .push_back((timestamp, metrics.average_response_time));
        if self.latency_samples.len() > self.max_samples {
            self.latency_samples.pop_front();
        }

        self.bandwidth_samples.push_back((
            timestamp,
            metrics.network_bandwidth_in,
            metrics.network_bandwidth_out,
        ));
        ...
        self.connection_counts
            .push_back((timestamp, metrics.active_connections));
