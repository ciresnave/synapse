...
async fn main() -> Result<()> {
    ...
    info!("💬 Starting Synapse Client v{}", env!("CARGO_PKG_VERSION"));
...
async fn send_message(
    ...
    match client.send_message(simple_msg, to.to_string()).await {
        ...
        Err(e @ synapse::error::SynapseError::KeyNotFound(_)) => {
            error!(
                "❌ No signing key loaded for {}. This CLI does not currently persist a keypair \
                 between invocations, so a keypair generated by one run cannot be seen by this \
                 one. Run this command from a process that has already generated or loaded a \
                 keypair, or supply one via configuration. Underlying error: {}",
...
async fn start_chat_session(client: SynapseRouter, with: &str) -> Result<()> {
    ...
    loop {
        ...
        match client.send_message(chat_message, with.to_string()).await {
            ...
            Err(e @ synapse::error::SynapseError::KeyNotFound(_)) => {
                error!(
                    "❌ No signing key loaded for {}. This CLI does not currently persist a \
                     keypair between invocations, so a keypair generated by one run cannot be \
                     seen by this one. Run this command from a process that has already \
                     generated or loaded a keypair, or supply one via configuration. Underlying \
                     error: {}",
                    client.get_our_global_id(),
                    e
                );
