...
>
> Measured at `f0f570c`: 31 occurrences across 19 files. Task 1 deleted the transport files that never compiled and the WASM files that were never declared as modules, which carried some of those placeholders along with them. Re-measured on this branch, after transport contract PR B, Task 8 rewrote `websocket_unified.rs` and removed its two: 15 occurrences remained across 9 files. Re-measured again on the `feat/router-merge` branch, after Task 6 deleted `src/router_enhanced.rs` (which carried one) as part of migrating its consumers to the merged `SynapseRouter` and removing the old router types entirely: **14 occurrences of `"In a real implementation"` remain across 8 files** in `src/`. *(Controls: the same query shape for `"In a ZzUnreal implementation"` returns 0, so it discriminates; the count was 46 at `7b80ca4`, 31 at `f0f570c`, and 15 before the router merge, so the reductions are genuine and the audit's work was real.)*
>
> **Survivors sit inside the subsystems this report declares complete:** `src/transport/nat_traversal.rs` (§44–48 STUN/TURN/ICE), `src/email.rs` ×2 (§68–72 SMTP/IMAP). Largest concentrations: `src/connectivity.rs` (6), `src/email.rs` (2), and six files with 1.
>
