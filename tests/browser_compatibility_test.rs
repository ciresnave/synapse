...
pub fn has_webrtc_support() -> bool {
    ...
    let media_devices =
        js_sys::Reflect::has(&window.navigator(), &JsValue::from_str("mediaDevices"))
            .unwrap_or(false);
...
fn test_webrtc_support() {
    ...
    console::log_1(&JsValue::from_str(
        "Note: WebRTC might be restricted in private browsing modes",
    ));
}
...
async fn test_local_storage() {
    ...
    assert_eq!(
        value, read_value,
        "localStorage value doesn't match expected value"
    );
...
fn test_wasm_crypto() {
    ...
    assert!(
        has_non_zero,
        "Crypto API should generate non-zero random values"
    );
}
...
impl WasmSynapseNode {
    ...
    pub fn get_capabilities(&self) -> JsValue {
        ...
        Reflect::set(
            ...
            &JsValue::from_bool(self.storage_supported),
        )
        .unwrap();
        Reflect::set(
            &obj,
            &JsValue::from_str("webrtc"),
            &JsValue::from_bool(self.webrtc_supported),
        )
        .unwrap();
        Reflect::set(
            &obj,
            &JsValue::from_str("websocket"),
            &JsValue::from_bool(has_websocket_support()),
        )
        .unwrap();
        Reflect::set(
            &obj,
            &JsValue::from_str("browser"),
            &JsValue::from_str(&detect_browser()),
        )
        .unwrap();
...
fn test_wasm_synapse_node() {
    ...
    assert!(
        node.is_browser_compatible(),
        "Browser should be compatible with Synapse WASM"
    );
}
