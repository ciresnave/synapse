...
mod concurrent_registry_tests {
    ...
    async fn test_concurrent_message_processing() -> Result<()> {
        ...
        assert!(
            success_count >= num_messages * 8 / 10,
            "At least 80% of concurrent message processing should succeed"
        );
        println!(
            "Successfully processed {success_count} out of {num_messages} messages concurrently"
        );
        Ok(())
    }
    ...
    async fn test_mixed_concurrent_operations() -> Result<()> {
        ...
        for i in (num_operations / 2)..num_operations {
            ...
            let task = tokio::spawn(async move {
                ...
                let _config =
                    Config::default_for_entity(format!("mixed_config_{i}"), "test".to_string());
        ...
        assert!(
            success_count >= num_operations * 7 / 10,
            "At least 70% of mixed concurrent operations should succeed"
        );
        println!(
            "Successfully completed {success_count} out of {num_operations} mixed operations concurrently"
        );
        Ok(())
    }
}
