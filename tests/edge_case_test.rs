...
mod edge_case_tests {
    ...
    async fn test_special_content_cases() -> Result<()> {
        ...
        for (test_name, content) in test_cases {
            ...
            assert!(
                result.is_ok(),
                "Special content case '{test_name}' should be handled"
            );
        }
    ...
    async fn test_entity_name_edge_cases() -> Result<()> {
        ...
        for identifier in edge_case_identifiers {
            ...
            match result {
                Ok(_router) => {
                    println!("✅ Entity identifier '{identifier}' accepted");
                }
                Err(e) => {
                    println!("❌ Entity identifier '{identifier}' rejected: {e}");
    ...
    async fn test_timeout_handling() -> Result<()> {
        ...
        let result = timeout(
            ...
            router.convert_to_secure_message(&msg),
        )
        .await;
        assert!(
            result.is_ok(),
            "Normal message conversion should complete within timeout"
        );
    ...
    async fn test_rapid_message_creation() -> Result<()> {
        ...
        match result {
            Ok(results) => {
                ...
                println!(
                    "Successfully processed {success_count} out of {message_count} rapid messages"
                );
                assert!(
                    success_count >= message_count * 8 / 10,
                    "At least 80% of rapid messages should succeed"
                );
            }
            Err(_) => {
    ...
    async fn test_message_type_variations() -> Result<()> {
        ...
        for msg_type in message_types {
            ...
            assert!(
                result.is_ok(),
                "Message type {msg_type:?} should be handled"
            );
        }
    ...
    async fn test_error_recovery() -> Result<()> {
        ...
        for (scenario_name, content) in scenarios {
            ...
            assert!(
                result.is_ok(),
                "Recovery scenario '{scenario_name}' should succeed"
            );
        }
