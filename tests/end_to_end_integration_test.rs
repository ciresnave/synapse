...
use synapse::{
    ...
    services::{registry::RegistryService, trust_manager::TrustManager},
    storage::database::Database,
    synapse::transport::error_recovery::{CircuitBreaker, ConnectionHealthMonitor, RetryPolicy},
    transport::{
        MessageDeliveryStatus, MessageUrgency, Transport, TransportError, TransportResult,
...
async fn test_end_to_end_with_error_recovery() -> Result<()> {
    ...
    // Test 1: Normal communication - all systems operational
    {
        ...
        let result = alice_transport_manager
            .send_message(&bob_id, message.as_bytes(), MessageUrgency::Interactive)
            .await;

        assert!(
            result.is_ok(),
            "Message should be delivered successfully under normal conditions"
        );

        let delivery = result.unwrap();
        assert_eq!(
            delivery.transport_id, "alice_primary",
            "Should use primary transport"
        );
    ...
    // Test 2: Primary transport failure with automatic failover
    {
        ...
        if let Some(transport) = alice_primary
            .as_any()
            .downcast_ref::<MockNetworkTransport>()
        {
            let mut transport = unsafe {
                &mut *(transport as *const MockNetworkTransport as *mut MockNetworkTransport)
            };
        ...
        assert!(
            result.is_ok(),
            "Message should be delivered successfully via failover"
        );

        let delivery = result.unwrap();
        assert_eq!(
            delivery.transport_id, "alice_secondary",
            "Should use secondary transport"
        );
        assert_eq!(delivery.recipient, bob_id, "Recipient should be Bob");
        assert!(delivery.delivered, "Message should be delivered");
    ...
    // Test 3: High latency scenario with circuit breaker
    {
        ...
        assert!(
            result.is_ok(),
            "Message should be delivered via secondary after circuit breaker trips"
        );
        assert_eq!(
            result.unwrap().transport_id,
            "alice_secondary",
            "Should use secondary transport due to circuit breaker"
        );
    ...
    // Test 4: Complete network partition and retry policy
    {
        ...
        if let Some(transport) = alice_secondary
            .as_any()
            .downcast_ref::<MockNetworkTransport>()
        {
            let mut transport = unsafe {
        ...
        if let Some(transport) = alice_fallback
            .as_any()
            .downcast_ref::<MockNetworkTransport>()
        {
            let mut transport = unsafe {
        ...
        assert!(
            result.is_err(),
            "Message should fail after retry policy exhaustion"
        );

        // Reset all transports
        for transport in [
            alice_primary.clone(),
            alice_secondary.clone(),
            alice_fallback.clone(),
        ] {
    ...
    // Test 5: Recovery after network partition
    {
        ...
        assert!(
            result.is_ok(),
            "Message should be delivered after network recovery"
        );
        assert_eq!(
            result.unwrap().transport_id,
            "alice_primary",
            "Should use primary transport after recovery"
        );
    }
